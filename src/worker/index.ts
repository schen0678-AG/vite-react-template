...
import { loadState, runAssistantTurn } from "./assistant";
...
// One conversational turn. Loads current state, runs the Sonnet 4.6 tool loop,
// returns the ops applied + the assistant's natural-language reply + new state.
app.post("/api/assistant/turn", async (c) => {
  const { text, language } = await c.req.json<{ text: string; language?: string }>();
  const userId = c.get("user").sub;
  try {
    const result = await runAssistantTurn(c.env, userId, text, language || "en-US");
    return c.json(result);
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
    const status = /too long|required/i.test(message) ? 400 : 500;
    return c.json({ error: message }, status);
  }
});

// Hydrate the UI on load / after manual edits.
app.get("/api/assistant/state", async (c) => {
  const userId = c.get("user").sub;
  const state = await loadState(c.env.DB, userId);
  return c.json(state);
});

// Manual edits from the UI (checkbox, trash icons). Authoritative side does
// the row-level user_id check on every WHERE clause.
app.patch("/api/assistant/todos/:id", async (c) => {
  const userId = c.get("user").sub;
  ...
  if (typeof patch.status === "string") {
    if (!allowedStatuses.has(patch.status)) return c.json({ error: "invalid status" }, 400);
    fields.push("status = ?"); values.push(patch.status);
  }
  if (patch.due_at === null || typeof patch.due_at === "string") {
    fields.push("due_at = ?"); values.push(patch.due_at ?? null);
  }
  if (typeof patch.action === "string") { fields.push("action = ?"); values.push(patch.action); }
  if (typeof patch.target === "string") { fields.push("target = ?"); values.push(patch.target); }
  if (typeof patch.notes === "string")  { fields.push("notes = ?");  values.push(patch.notes); }
...
app.delete("/api/assistant/todos/:id", async (c) => {
  const userId = c.get("user").sub;
  const id = Number(c.req.param("id"));
  const res = await c.env.DB.prepare("DELETE FROM todos WHERE id = ? AND user_id = ?")
    .bind(id, userId)
    .run();
  return c.json({ ok: true, changes: res.meta.changes ?? 0 });
});

app.delete("/api/assistant/projects/:id", async (c) => {
  const userId = c.get("user").sub;
  const id = Number(c.req.param("id"));
  const res = await c.env.DB.prepare("DELETE FROM projects WHERE id = ? AND user_id = ?")
    .bind(id, userId)
    .run();
  return c.json({ ok: true, changes: res.meta.changes ?? 0 });
});

app.delete("/api/assistant/features/:id", async (c) => {
  const userId = c.get("user").sub;
  const id = Number(c.req.param("id"));
  const res = await c.env.DB.prepare("DELETE FROM features WHERE id = ? AND user_id = ?")
    .bind(id, userId)
    .run();
  return c.json({ ok: true, changes: res.meta.changes ?? 0 });
});
